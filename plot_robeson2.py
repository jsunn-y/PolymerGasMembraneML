...
filelist= ['models/RF_BLR_fing/Y_pred_datasetCX_fing_0.csv', 'models/RF_BLR_fing/Y_pred_datasetBX_fing.csv','models/RF_BLR_fing/Y_pred_datasetDX_fing.csv', 'models/RF_BLR_fing/Y_train.csv']
...
for i, dataset in enumerate(filelist[:-1]):
    ...
    if i == 0:
        for j in range(8):
            new = pd.read_csv('models/RF_BLR_fing/Y_pred_datasetCX_fing_' + str(j+1) + '.csv', header=None)
            Y_pred = pd.concat([Y_pred, new])
...
plt.savefig('predictionsRF.jpg', dpi=300)
